...
//dump to buffer. DLL remains unchanged
char* readDll(dll* myList){
		  int lengthOfList = countDll(myList);
		  char *buff = malloc(sizeof(char)*(lengthOfList+1));
		  dllNode* travel = myList->head->next;
		  int i = 0;
		  char rippedChar;

		  while(travel != myList->tail){
					 rippedChar = travel->character;
					 *(buff + i) = rippedChar;
